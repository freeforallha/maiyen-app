...
class _HomePageState extends State<HomePage> {
  ...
  Color getHomeColor(String h) {
    ...
    if (selected) {
      return unsafe ? Colors.red : Colors.green;
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final openList = devices.entries.map((e) {
      final d = e.value;
      final name = d["name"]?.toString() ?? e.key;
      final status = d["status"] == "closed" ? "đóng" : "mở";
      final tamper = d["tamper"] == true ? "Bất thường" : "Bình thường";
      return "🚪 $name ($status) - Tamper: $tamper";
    }).toList();
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  unsafe ? "KHÔNG AN TOÀN" : "ĐÃ AN TOÀN",
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: unsafe ? Colors.red : Colors.green,
                  ),
                ),
                ...openList.map((e) => Text(e)),
              ],
            ),
          ),
