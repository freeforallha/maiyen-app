const Map<String, String> roDynamicStrings = {
  "Bảo vệ - Tự động": "Protecție — Automată",
  "Bảo vệ - Thủ công": "Protecție — Manuală",
  "Tự động": "Automată",
  "Thủ công": "Manuală",
  "\$displayMemberName đã gia nhập nhà \"\$homeName\".":
      "\$displayMemberName s-a alăturat locuinței „\$homeName”.",
  "NGUY HIỂM": "PERICOL",
  "Nguy hiểm & Khẩn cấp": "Pericol și urgență",
  "TẮT CÒI": "OPREȘTE SIRENA",
  "Tắt còi báo động?": "Oprești sirena?",
  "Còi vật lý sẽ dừng, nhưng cảnh báo vẫn tiếp tục cho đến khi sự cố được xử lý.\n\nBạn chắc chắn muốn tắt còi?":
      "Sirena fizică se va opri, dar alerta va continua până când problema este rezolvată.\n\nSigur vrei să oprești sirena?",
  "Đã gửi lệnh tắt còi. Cảnh báo vẫn đang được theo dõi.":
      "Comanda de oprire a sirenei a fost trimisă. Alerta este în continuare monitorizată.",
  "Không tìm thấy cảnh báo đang hoạt động hoặc chưa thể gửi lệnh tắt còi.":
      "Nu a fost găsită nicio alertă activă sau comanda de oprire a sirenei nu a putut fi trimisă încă.",
  "TẮT CÒI TRONG NHÀ": "OPREȘTE SIRENA DIN LOCUINȚĂ",
  "Đã tắt còi trong nhà. Cảnh báo vẫn tiếp tục cho đến khi được xử lý.":
      "Sirena din locuință a fost oprită. Alerta va continua până când problema este rezolvată.",
  "\$name đang chuẩn bị gửi tin...": "\$name scrie...",
  "\$name1 và \$name2 đang chuẩn bị gửi tin...": "\$name1 și \$name2 scriu...",
  "\$name và \$otherCount người khác đang chuẩn bị gửi tin...":
      "\$name și încă \$otherCount persoane scriu...",
  "Kênh báo động cũ để giữ tương thích":
      "Canal vechi de alarmă păstrat pentru compatibilitate",
  "MaiYen báo động toàn màn hình": "Alarmă MaiYen pe tot ecranul",
  "Mở cảnh báo toàn màn hình; âm còi phát từ trang báo động":
      "Deschide alerta pe tot ecranul; sunetul sirenei este redat din pagina Alarmei",
  "MaiYen cảnh báo khẩn cấp": "Alertă de urgență MaiYen",
  "Cảnh báo khẩn cấp ưu tiên cao trước khi mở toàn màn hình":
      "Alertă de urgență cu prioritate ridicată înainte de deschiderea pe tot ecranul",
  "MaiYen nhắc nhở toàn màn hình": "Memento MaiYen pe tot ecranul",
  "Nhắc nhở MaiYen toàn màn hình không âm thanh":
      "Memento MaiYen pe tot ecranul, fără sunet",
  "MaiYen nhắc nhở ưu tiên cao": "Memento MaiYen cu prioritate ridicată",
  "Nhắc nhở MaiYen ưu tiên cao, không mở toàn màn hình":
      "Memento MaiYen cu prioritate ridicată, fără deschidere pe tot ecranul",
  "Tin nhắn mới trong các nhà MaiYen": "Mesaje noi în locuințele MaiYen",
  "Hành động này sẽ thay đổi thời gian báo động của một số thiết bị hôm nay.\n\nBáo động của các thiết bị thuộc trường \"Nguy hiểm khẩn cấp\" và báo động ở chế độ \"Bảo vệ\" sẽ không bị ảnh hưởng bởi chức năng này.":
      "Această acțiune va modifica astăzi ora alarmei pentru unele dispozitive.\n\nAlarmele dispozitivelor din categoria „Pericol de urgență” și alarmele din modul „Protecție” nu vor fi afectate de această funcție.",
  "🆘 \$count nhà nguy hiểm\$suffix": "🆘 \$count locuințe în pericol\$suffix",
  "FCM token đã sẵn sàng, nhưng tính năng tự động khi rời nhà còn thiếu điều kiện.":
      "Tokenul FCM este pregătit, dar funcției automate la plecare îi lipsește o cerință.",
  "Đã kích hoạt SOS": "SOS activat",
  "Phát hiện chập điện": "Scurtcircuit detectat",
  "Phát hiện quá dòng": "Supracurent detectat",
  "Phát hiện quá áp": "Supratensiune detectată",
  "Thiết bị điện quá nhiệt": "Dispozitiv electric supraîncălzit",
  "Tài khoản đang được sử dụng": "Contul este utilizat",
  "Hiện tại tài khoản này đang đăng nhập trên một thiết bị khác. Nếu tiếp tục, tài khoản trên thiết bị đó sẽ tự đăng xuất.":
      "Acest cont este conectat în prezent pe un alt dispozitiv. Dacă continui, contul de pe acel dispozitiv va fi deconectat automat.",
  "Tiếp tục đăng nhập": "Continuă autentificarea",
  "Tài khoản đã được đăng nhập trên một thiết bị khác.":
      "Contul a fost autentificat pe un alt dispozitiv.",
  "KHẨN CẤP": "URGENȚĂ",
  "CẦN CHÚ Ý": "NECESITĂ ATENȚIE",
  "THÔNG TIN": "INFORMAȚII",
  "BÁO ĐỘNG": "ALARMĂ",
  "Cảm biến đã trở lại trạng thái an toàn.":
      "Senzorul a revenit la starea sigură.",
  "Người dùng đã tắt cảnh báo.": "Utilizatorul a oprit alerta.",
  "Khung giờ bảo vệ đã kết thúc.": "Intervalul de Protecție s-a încheiat.",
  "Cảnh báo tức thời đã tự kết thúc.":
      "Alerta instantanee s-a încheiat automat.",
  "Điều kiện cảnh báo không còn hoạt động.":
      "Condiția de alertă nu mai este activă.",
  "Cảnh báo đã kết thúc: \$reason": "Alerta s-a încheiat: \$reason",
};
