const Map<String, String> plDynamicStrings = {
  "Bảo vệ - Tự động": "Ochrona - Automatyczna",
  "Bảo vệ - Thủ công": "Ochrona - Ręczna",
  "Tự động": "Automatyczna",
  "Thủ công": "Ręczna",
  "\$displayMemberName đã gia nhập nhà \"\$homeName\".":
      "Odnotowano dołączenie użytkownika \$displayMemberName do domu „\$homeName”.",
  "NGUY HIỂM": "NIEBEZPIECZEŃSTWO",
  "Nguy hiểm & Khẩn cấp": "Zagrożenie i sytuacja awaryjna",
  "TẮT CÒI": "WYŁĄCZ SYRENĘ",
  "Tắt còi báo động?": "Wyłączyć syrenę alarmową?",
  "Còi vật lý sẽ dừng, nhưng cảnh báo vẫn tiếp tục cho đến khi sự cố được xử lý.\n\nBạn chắc chắn muốn tắt còi?":
      "Fizyczna syrena zostanie wyłączona, ale alert będzie trwał do rozwiązania problemu.\n\nCzy na pewno chcesz wyłączyć syrenę?",
  "Đã gửi lệnh tắt còi. Cảnh báo vẫn đang được theo dõi.":
      "Wysłano polecenie wyłączenia syreny. Alert jest nadal monitorowany.",
  "Không tìm thấy cảnh báo đang hoạt động hoặc chưa thể gửi lệnh tắt còi.":
      "Nie znaleziono aktywnego alertu lub nie można jeszcze wysłać polecenia wyłączenia syreny.",
  "TẮT CÒI TRONG NHÀ": "WYŁĄCZ SYRENĘ W DOMU",
  "Đã tắt còi trong nhà. Cảnh báo vẫn tiếp tục cho đến khi được xử lý.":
      "Syrena w domu została wyłączona. Alert będzie trwał do rozwiązania problemu.",
  "\$name đang chuẩn bị gửi tin...": "\$name pisze...",
  "\$name1 và \$name2 đang chuẩn bị gửi tin...": "\$name1 i \$name2 piszą...",
  "\$name và \$otherCount người khác đang chuẩn bị gửi tin...":
      "\$name i \$otherCount innych osób piszą...",
  "Kênh báo động cũ để giữ tương thích":
      "Starszy kanał alarmowy zachowany dla zgodności",
  "MaiYen báo động toàn màn hình": "Pełnoekranowy alarm MaiYen",
  "Mở cảnh báo toàn màn hình; âm còi phát từ trang báo động":
      "Otwiera alert pełnoekranowy; dźwięk syreny jest odtwarzany na ekranie alarmu",
  "MaiYen cảnh báo khẩn cấp": "Alert awaryjny MaiYen",
  "Cảnh báo khẩn cấp ưu tiên cao trước khi mở toàn màn hình":
      "Alert awaryjny o wysokim priorytecie przed otwarciem pełnego ekranu",
  "MaiYen nhắc nhở toàn màn hình": "Pełnoekranowe przypomnienie MaiYen",
  "Nhắc nhở MaiYen toàn màn hình không âm thanh":
      "Ciche pełnoekranowe przypomnienie MaiYen",
  "MaiYen nhắc nhở ưu tiên cao":
      "Przypomnienie MaiYen o wysokim priorytecie",
  "Nhắc nhở MaiYen ưu tiên cao, không mở toàn màn hình":
      "Przypomnienie MaiYen o wysokim priorytecie bez otwierania pełnego ekranu",
  "Tin nhắn mới trong các nhà MaiYen": "Nowe wiadomości w domach MaiYen",
  "Hành động này sẽ thay đổi thời gian báo động của một số thiết bị hôm nay.\n\nBáo động của các thiết bị thuộc trường \"Nguy hiểm khẩn cấp\" và báo động ở chế độ \"Bảo vệ\" sẽ không bị ảnh hưởng bởi chức năng này.":
      "Ta czynność zmieni dziś godzinę alarmu niektórych urządzeń.\n\nAlarmy urządzeń z kategorii „Zagrożenie awaryjne” oraz alarmy w trybie „Ochrona” nie zostaną objęte tą funkcją.",
  "🆘 \$count nhà nguy hiểm\$suffix":
      "🆘 \$count domów w niebezpieczeństwie\$suffix",
  "FCM token đã sẵn sàng, nhưng tính năng tự động khi rời nhà còn thiếu điều kiện.":
      "Token FCM jest gotowy, ale funkcja automatycznej ochrony po wyjściu z domu nie spełnia jeszcze jednego warunku.",
  "Đã kích hoạt SOS": "Aktywowano SOS",
  "Phát hiện chập điện": "Wykryto zwarcie",
  "Phát hiện quá dòng": "Wykryto przetężenie",
  "Phát hiện quá áp": "Wykryto przepięcie",
  "Thiết bị điện quá nhiệt": "Przegrzanie urządzenia elektrycznego",
  "Tài khoản đang được sử dụng": "Konto jest używane",
  "Hiện tại tài khoản này đang đăng nhập trên một thiết bị khác. Nếu tiếp tục, tài khoản trên thiết bị đó sẽ tự đăng xuất.":
      "To konto jest obecnie zalogowane na innym urządzeniu. Jeśli kontynuujesz, konto na tamtym urządzeniu zostanie automatycznie wylogowane.",
  "Tiếp tục đăng nhập": "Kontynuuj logowanie",
  "Tài khoản đã được đăng nhập trên một thiết bị khác.":
      "Konto zostało zalogowane na innym urządzeniu.",
  "KHẨN CẤP": "SYTUACJA AWARYJNA",
  "CẦN CHÚ Ý": "WYMAGA UWAGI",
  "THÔNG TIN": "INFORMACJA",
  "BÁO ĐỘNG": "ALARM",
  "Cảm biến đã trở lại trạng thái an toàn.":
      "Czujnik wrócił do bezpiecznego stanu.",
  "Người dùng đã tắt cảnh báo.": "Użytkownik wyłączył alert.",
  "Khung giờ bảo vệ đã kết thúc.": "Przedział ochrony zakończył się.",
  "Cảnh báo tức thời đã tự kết thúc.":
      "Natychmiastowy alert zakończył się automatycznie.",
  "Điều kiện cảnh báo không còn hoạt động.":
      "Warunek alertu nie jest już aktywny.",
  "Cảnh báo đã kết thúc: \$reason": "Alert zakończony: \$reason",
};
