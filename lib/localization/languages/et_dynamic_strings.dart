const Map<String, String> etDynamicStrings = {
  "Bảo vệ - Tự động": "Kaitse – automaatne",
  "Bảo vệ - Thủ công": "Kaitse – käsitsi",
  "Tự động": "Automaatne",
  "Thủ công": "Käsitsi",
  "\$displayMemberName đã gia nhập nhà \"\$homeName\".":
      "\$displayMemberName liitus koduga „\$homeName“.",
  "NGUY HIỂM": "OHT",
  "Nguy hiểm & Khẩn cấp": "Oht ja hädaolukord",
  "TẮT CÒI": "LÜLITA SIREEN VÄLJA",
  "Tắt còi báo động?": "Lülitada alarmisireen välja?",
  "Còi vật lý sẽ dừng, nhưng cảnh báo vẫn tiếp tục cho đến khi sự cố được xử lý.\n\nBạn chắc chắn muốn tắt còi?":
      "Füüsiline sireen peatub, kuid hoiatus jätkub, kuni juhtum on lahendatud.\n\nKas oled kindel, et soovid sireeni välja lülitada?",
  "Đã gửi lệnh tắt còi. Cảnh báo vẫn đang được theo dõi.":
      "Sireeni väljalülitamise käsk saadeti. Hoiatust jälgitakse endiselt.",
  "Không tìm thấy cảnh báo đang hoạt động hoặc chưa thể gửi lệnh tắt còi.":
      "Aktiivset hoiatust ei leitud või sireeni väljalülitamise käsku ei saanud saata.",
  "TẮT CÒI TRONG NHÀ": "LÜLITA KODU SIREEN VÄLJA",
  "Đã tắt còi trong nhà. Cảnh báo vẫn tiếp tục cho đến khi được xử lý.":
      "Kodu sireen lülitati välja. Hoiatus jätkub, kuni juhtum on lahendatud.",
  "\$name đang chuẩn bị gửi tin...": "\$name kirjutab...",
  "\$name1 và \$name2 đang chuẩn bị gửi tin...":
      "\$name1 ja \$name2 kirjutavad...",
  "\$name và \$otherCount người khác đang chuẩn bị gửi tin...":
      "\$name ja veel \$otherCount inimest kirjutavad...",
  "Kênh báo động cũ để giữ tương thích":
      "Ühilduvuse säilitamiseks jäetud vana alarmikanal",
  "SafeHome báo động toàn màn hình": "SafeHome'i täisekraanialarm",
  "Mở cảnh báo toàn màn hình; âm còi phát từ trang báo động":
      "Avab täisekraanihoiatuse; sireeniheli esitatakse alarmilehelt",
  "SafeHome cảnh báo khẩn cấp": "SafeHome'i hädaolukorra hoiatus",
  "Cảnh báo khẩn cấp ưu tiên cao trước khi mở toàn màn hình":
      "Kõrge prioriteediga hädaolukorra hoiatus enne täisekraani avamist",
  "SafeHome nhắc nhở toàn màn hình": "SafeHome'i täisekraani meeldetuletus",
  "Nhắc nhở SafeHome toàn màn hình không âm thanh":
      "Helitu SafeHome'i täisekraani meeldetuletus",
  "SafeHome nhắc nhở ưu tiên cao":
      "SafeHome'i kõrge prioriteediga meeldetuletus",
  "Nhắc nhở SafeHome ưu tiên cao, không mở toàn màn hình":
      "Kõrge prioriteediga SafeHome'i meeldetuletus ilma täisekraanita",
  "Tin nhắn mới trong các nhà SafeHome": "Uued sõnumid SafeHome'i kodudes",
  "Hành động này sẽ thay đổi thời gian báo động của một số thiết bị hôm nay.\n\nBáo động của các thiết bị thuộc trường \"Nguy hiểm khẩn cấp\" và báo động ở chế độ \"Bảo vệ\" sẽ không bị ảnh hưởng bởi chức năng này.":
      "See toiming muudab täna mõne seadme alarmiaega.\n\nKategooria „Hädaolukorra oht“ seadmete alarmid ja režiimi „Kaitse“ alarmid jäävad sellest funktsioonist puutumata.",
  "🆘 \$count nhà nguy hiểm\$suffix": "🆘 \$count kodu ohus\$suffix",
  "FCM token đã sẵn sàng, nhưng tính năng tự động khi rời nhà còn thiếu điều kiện.":
      "FCM-i tunnus on valmis, kuid automaatse eemalolekurežiimi jaoks puudub üks nõue.",
  "Đã kích hoạt SOS": "SOS aktiveeritud",
  "Phát hiện chập điện": "Tuvastati lühis",
  "Phát hiện quá dòng": "Tuvastati ülevool",
  "Phát hiện quá áp": "Tuvastati ülepinge",
  "Thiết bị điện quá nhiệt": "Elektriseade on üle kuumenenud",
  "Tài khoản đang được sử dụng": "Konto on kasutusel",
  "Hiện tại tài khoản này đang đăng nhập trên một thiết bị khác. Nếu tiếp tục, tài khoản trên thiết bị đó sẽ tự đăng xuất.":
      "See konto on praegu sisse logitud teises seadmes. Kui jätkad, logitakse konto selles seadmes automaatselt välja.",
  "Tiếp tục đăng nhập": "Jätka sisselogimist",
  "Tài khoản đã được đăng nhập trên một thiết bị khác.":
      "Kontole logiti sisse teises seadmes.",
  "KHẨN CẤP": "HÄDAOLUKORD",
  "CẦN CHÚ Ý": "VAJAB TÄHELEPANU",
  "THÔNG TIN": "TEAVE",
  "BÁO ĐỘNG": "ALARM",
  "Cảm biến đã trở lại trạng thái an toàn.":
      "Andur on naasnud ohutusse seisundisse.",
  "Người dùng đã tắt cảnh báo.": "Kasutaja lülitas hoiatuse välja.",
  "Khung giờ bảo vệ đã kết thúc.": "Kaitseperiood on lõppenud.",
  "Cảnh báo tức thời đã tự kết thúc.": "Kohene hoiatus lõppes automaatselt.",
  "Điều kiện cảnh báo không còn hoạt động.":
      "Hoiatuse tingimus pole enam aktiivne.",
  "Cảnh báo đã kết thúc: \$reason": "Hoiatus lõppes: \$reason",
};
