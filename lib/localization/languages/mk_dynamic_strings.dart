const Map<String, String> mkDynamicStrings = {
  "Bảo vệ - Tự động": "Заштита – Автоматска",
  "Bảo vệ - Thủ công": "Заштита – Рачна",
  "Tự động": "Автоматски",
  "Thủ công": "Рачно",
  "\$displayMemberName đã gia nhập nhà \"\$homeName\".":
      "\$displayMemberName се придружи на домот „\$homeName“.",
  "NGUY HIỂM": "ОПАСНОСТ",
  "Nguy hiểm & Khẩn cấp": "Опасност и итни случаи",
  "TẮT CÒI": "ИСКЛУЧИ ЈА СИРЕНАТА",
  "Tắt còi báo động?": "Да се исклучи сирената?",
  "Còi vật lý sẽ dừng, nhưng cảnh báo vẫn tiếp tục cho đến khi sự cố được xử lý.\n\nBạn chắc chắn muốn tắt còi?":
      "Физичката сирена ќе запре, но предупредувањето ќе остане активно додека не се реши проблемот.\n\nДали сте сигурни дека сакате да ја исклучите сирената?",
  "Đã gửi lệnh tắt còi. Cảnh báo vẫn đang được theo dõi.":
      "Испратена е командата за исклучување на сирената. Предупредувањето и понатаму се следи.",
  "Không tìm thấy cảnh báo đang hoạt động hoặc chưa thể gửi lệnh tắt còi.":
      "Не е пронајдено активно предупредување или командата за исклучување на сирената сè уште не може да се испрати.",
  "TẮT CÒI TRONG NHÀ": "ИСКЛУЧИ ЈА СИРЕНАТА ВО ДОМОТ",
  "Đã tắt còi trong nhà. Cảnh báo vẫn tiếp tục cho đến khi được xử lý.":
      "Сирената во домот е исклучена. Предупредувањето останува активно додека не се реши проблемот.",
  "\$name đang chuẩn bị gửi tin...": "\$name пишува...",
  "\$name1 và \$name2 đang chuẩn bị gửi tin...":
      "\$name1 и \$name2 пишуваат...",
  "\$name và \$otherCount người khác đang chuẩn bị gửi tin...":
      "\$name и уште \$otherCount други пишуваат...",
  "Kênh báo động cũ để giữ tương thích":
      "Стар канал за аларм заради компатибилност",
  "SafeHome báo động toàn màn hình": "Аларм SafeHome на цел екран",
  "Mở cảnh báo toàn màn hình; âm còi phát từ trang báo động":
      "Отвора предупредување на цел екран; звукот на сирената се репродуцира од страницата за аларм",
  "SafeHome cảnh báo khẩn cấp": "Итно предупредување SafeHome",
  "Cảnh báo khẩn cấp ưu tiên cao trước khi mở toàn màn hình":
      "Итно предупредување со висок приоритет пред отворање на цел екран",
  "SafeHome nhắc nhở toàn màn hình": "Потсетник SafeHome на цел екран",
  "Nhắc nhở SafeHome toàn màn hình không âm thanh":
      "Тивок потсетник SafeHome на цел екран",
  "SafeHome nhắc nhở ưu tiên cao": "Потсетник SafeHome со висок приоритет",
  "Nhắc nhở SafeHome ưu tiên cao, không mở toàn màn hình":
      "Потсетник SafeHome со висок приоритет без отворање на цел екран",
  "Tin nhắn mới trong các nhà SafeHome": "Нови пораки во домовите SafeHome",
  "Hành động này sẽ thay đổi thời gian báo động của một số thiết bị hôm nay.\n\nBáo động của các thiết bị thuộc trường \"Nguy hiểm khẩn cấp\" và báo động ở chế độ \"Bảo vệ\" sẽ không bị ảnh hưởng bởi chức năng này.":
      "Ова дејство денес ќе го промени времето на алармот за некои уреди.\n\nОваа функција нема да влијае на алармите од уредите во категоријата „Итна опасност“ ниту на алармите во режимот „Заштита“.",
  "🆘 \$count nhà nguy hiểm\$suffix": "🆘 \$count домови во опасност\$suffix",
  "FCM token đã sẵn sàng, nhưng tính năng tự động khi rời nhà còn thiếu điều kiện.":
      "FCM-токенот е подготвен, но недостасува услов за автоматската заштита при заминување.",
  "Đã kích hoạt SOS": "SOS е активиран",
  "Phát hiện chập điện": "Откриен е краток спој",
  "Phát hiện quá dòng": "Откриена е прекумерна струја",
  "Phát hiện quá áp": "Откриен е пренапон",
  "Thiết bị điện quá nhiệt": "Електричниот уред е прегреан",
  "Tài khoản đang được sử dụng": "Сметката се користи",
  "Hiện tại tài khoản này đang đăng nhập trên một thiết bị khác. Nếu tiếp tục, tài khoản trên thiết bị đó sẽ tự đăng xuất.":
      "Оваа сметка моментално е најавена на друг уред. Ако продолжите, сметката на тој уред автоматски ќе се одјави.",
  "Tiếp tục đăng nhập": "Продолжи со најавување",
  "Tài khoản đã được đăng nhập trên một thiết bị khác.":
      "Сметката е најавена на друг уред.",
  "KHẨN CẤP": "ИТНО",
  "CẦN CHÚ Ý": "ПОТРЕБНО Е ВНИМАНИЕ",
  "THÔNG TIN": "ИНФОРМАЦИИ",
  "BÁO ĐỘNG": "АЛАРМ",
  "Cảm biến đã trở lại trạng thái an toàn.":
      "Сензорот се врати во безбедна состојба.",
  "Người dùng đã tắt cảnh báo.": "Корисникот го исклучи предупредувањето.",
  "Khung giờ bảo vệ đã kết thúc.": "Периодот на заштита заврши.",
  "Cảnh báo tức thời đã tự kết thúc.":
      "Непосредното предупредување заврши автоматски.",
  "Điều kiện cảnh báo không còn hoạt động.":
      "Условот за предупредување повеќе не е активен.",
  "Cảnh báo đã kết thúc: \$reason": "Предупредувањето заврши: \$reason",
};
