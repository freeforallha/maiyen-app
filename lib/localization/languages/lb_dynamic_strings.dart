const Map<String, String> lbDynamicStrings = {
  "Bảo vệ - Tự động": "Schutz – Automatesch",
  "Bảo vệ - Thủ công": "Schutz – Manuell",
  "Tự động": "Automatesch",
  "Thủ công": "Manuell",
  "\$displayMemberName đã gia nhập nhà \"\$homeName\".":
      "\$displayMemberName ass dem Doheem \"\$homeName\" bäigetrueden.",
  "NGUY HIỂM": "GEFOR",
  "Nguy hiểm & Khẩn cấp": "Gefor & Noutfall",
  "TẮT CÒI": "SIRÈNE AUSMAACHEN",
  "Tắt còi báo động?": "Alarmsirène ausmaachen?",
  "Còi vật lý sẽ dừng, nhưng cảnh báo vẫn tiếp tục cho đến khi sự cố được xử lý.\n\nBạn chắc chắn muốn tắt còi?":
      "D'physesch Sirène hält op, mee d'Warnung bleift aktiv bis den Tëschefall geléist ass.\n\nSidd Dir sécher, datt Dir d'Sirène ausmaache wëllt?",
  "Đã gửi lệnh tắt còi. Cảnh báo vẫn đang được theo dõi.":
      "De Kommando fir d'Sirène auszemaachen ass geschéckt. D'Warnung gëtt weider iwwerwaacht.",
  "Không tìm thấy cảnh báo đang hoạt động hoặc chưa thể gửi lệnh tắt còi.":
      "Keng aktiv Warnung fonnt oder de Kommando fir d'Sirène auszemaache konnt net geschéckt ginn.",
  "TẮT CÒI TRONG NHÀ": "SIRÈNE AM DOHEEM AUSMAACHEN",
  "Đã tắt còi trong nhà. Cảnh báo vẫn tiếp tục cho đến khi được xử lý.":
      "D'Sirène am Doheem ass aus. D'Warnung bleift aktiv bis se geléist ass.",
  "\$name đang chuẩn bị gửi tin...": "\$name schreift...",
  "\$name1 và \$name2 đang chuẩn bị gửi tin...":
      "\$name1 an \$name2 schreiwen...",
  "\$name và \$otherCount người khác đang chuẩn bị gửi tin...":
      "\$name an \$otherCount aner schreiwen...",
  "Kênh báo động cũ để giữ tương thích":
      "Alen Alarmkanal fir Kompatibilitéit bäibehalen",
  "SafeHome báo động toàn màn hình": "SafeHome-Alarm am Vollbild",
  "Mở cảnh báo toàn màn hình; âm còi phát từ trang báo động":
      "Vollbildwarnung opmaachen; de Sirèneklang gëtt vun der Alarmsäit ofgespillt",
  "SafeHome cảnh báo khẩn cấp": "SafeHome-Noutwarnung",
  "Cảnh báo khẩn cấp ưu tiên cao trước khi mở toàn màn hình":
      "Noutwarnung mat héijer Prioritéit virum Vollbild",
  "SafeHome nhắc nhở toàn màn hình": "SafeHome-Erënnerung am Vollbild",
  "Nhắc nhở SafeHome toàn màn hình không âm thanh":
      "Stomm SafeHome-Erënnerung am Vollbild",
  "SafeHome nhắc nhở ưu tiên cao": "SafeHome-Erënnerung mat héijer Prioritéit",
  "Nhắc nhở SafeHome ưu tiên cao, không mở toàn màn hình":
      "SafeHome-Erënnerung mat héijer Prioritéit ouni Vollbild",
  "Tin nhắn mới trong các nhà SafeHome": "Nei Messagen an de SafeHome-Doheemer",
  "Hành động này sẽ thay đổi thời gian báo động của một số thiết bị hôm nay.\n\nBáo động của các thiết bị thuộc trường \"Nguy hiểm khẩn cấp\" và báo động ở chế độ \"Bảo vệ\" sẽ không bị ảnh hưởng bởi chức năng này.":
      "Dës Aktioun ännert haut d’Alarmzäit vun e puer Apparater.\n\nAlarmer vun Apparater an der Kategorie „Akut Gefor“ an Alarmer am Modus „Schutz“ ginn duerch dës Funktioun net beaflosst.",
  "🆘 \$count nhà nguy hiểm\$suffix": "🆘 \$count Doheemer a Gefor\$suffix",
  "FCM token đã sẵn sàng, nhưng tính năng tự động khi rời nhà còn thiếu điều kiện.":
      "Den FCM-Token ass prett, mee fir d'automatesch Funktioun beim Verloosse vum Doheem feelt nach eng Konditioun.",
  "Đã kích hoạt SOS": "SOS aktivéiert",
  "Phát hiện chập điện": "Kuerzschluss erkannt",
  "Phát hiện quá dòng": "Iwwerstroum erkannt",
  "Phát hiện quá áp": "Iwwerspannung erkannt",
  "Thiết bị điện quá nhiệt": "Elektrescht Gerät iwwerhëtzt",
  "Tài khoản đang được sử dụng": "Kont gëtt benotzt",
  "Hiện tại tài khoản này đang đăng nhập trên một thiết bị khác. Nếu tiếp tục, tài khoản trên thiết bị đó sẽ tự đăng xuất.":
      "Dëse Kont ass aktuell op engem aneren Apparat ugemellt. Wann Dir weiderfuert, gëtt de Kont op deem Apparat automatesch ofgemellt.",
  "Tiếp tục đăng nhập": "Mat der Umeldung weiderfueren",
  "Tài khoản đã được đăng nhập trên một thiết bị khác.":
      "De Kont gouf op engem aneren Apparat ugemellt.",
  "KHẨN CẤP": "NOUTFALL",
  "CẦN CHÚ Ý": "BRAUCHT OPMIERKSAMKEET",
  "THÔNG TIN": "INFORMATIOUN",
  "BÁO ĐỘNG": "ALARM",
  "Cảm biến đã trở lại trạng thái an toàn.":
      "De Sensor ass an e sécheren Zoustand zeréckgaangen.",
  "Người dùng đã tắt cảnh báo.": "E Benotzer huet d'Warnung ausgeschalt.",
  "Khung giờ bảo vệ đã kết thúc.": "D'Schutzperiod ass eriwwer.",
  "Cảnh báo tức thời đã tự kết thúc.":
      "Déi direkt Warnung ass automatesch ofgeschloss ginn.",
  "Điều kiện cảnh báo không còn hoạt động.":
      "D'Warnbedingung ass net méi aktiv.",
  "Cảnh báo đã kết thúc: \$reason": "Warnung ofgeschloss: \$reason",
};
