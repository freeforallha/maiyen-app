const Map<String, String> gaDynamicStrings = {
  "Bảo vệ - Tự động": "Cosaint – Uathoibríoch",
  "Bảo vệ - Thủ công": "Cosaint – Lámhleabhar",
  "Tự động": "Uathoibríoch",
  "Thủ công": "Lámhleabhar",
  "\$displayMemberName đã gia nhập nhà \"\$homeName\".":
      "Chuaigh \$displayMemberName isteach i “\$homeName”.",
  "NGUY HIỂM": "CONTÚIRT",
  "Nguy hiểm & Khẩn cấp": "Contúirt agus éigeandáil",
  "TẮT CÒI": "MÚCH AN BONNÁN",
  "Tắt còi báo động?": "An bonnán aláraim a mhúchadh?",
  "Còi vật lý sẽ dừng, nhưng cảnh báo vẫn tiếp tục cho đến khi sự cố được xử lý.\n\nBạn chắc chắn muốn tắt còi?":
      "Stopfaidh an bonnán fisiciúil, ach leanfaidh an foláireamh go dtí go mbeidh an teagmhas láimhseáilte.\n\nAn bhfuil tú cinnte gur mhaith leat an bonnán a mhúchadh?",
  "Đã gửi lệnh tắt còi. Cảnh báo vẫn đang được theo dõi.":
      "Seoladh an t-ordú chun an bonnán a mhúchadh. Tá monatóireacht á déanamh ar an bhfoláireamh fós.",
  "Không tìm thấy cảnh báo đang hoạt động hoặc chưa thể gửi lệnh tắt còi.":
      "Níor aimsíodh foláireamh gníomhach nó níorbh fhéidir an t-ordú chun an bonnán a mhúchadh a sheoladh.",
  "TẮT CÒI TRONG NHÀ": "MÚCH BONNÁN AN BHAILE",
  "Đã tắt còi trong nhà. Cảnh báo vẫn tiếp tục cho đến khi được xử lý.":
      "Múchadh bonnán an bhaile. Leanfaidh an foláireamh go dtí go mbeidh an teagmhas láimhseáilte.",
  "\$name đang chuẩn bị gửi tin...": "Tá \$name ag clóscríobh...",
  "\$name1 và \$name2 đang chuẩn bị gửi tin...":
      "Tá \$name1 agus \$name2 ag clóscríobh...",
  "\$name và \$otherCount người khác đang chuẩn bị gửi tin...":
      "Tá \$name agus \$otherCount duine eile ag clóscríobh...",
  "Kênh báo động cũ để giữ tương thích":
      "Sean-chainéal aláraim coinnithe ar mhaithe le comhoiriúnacht",
  "SafeHome báo động toàn màn hình": "Aláram lánscáileáin SafeHome",
  "Mở cảnh báo toàn màn hình; âm còi phát từ trang báo động":
      "Osclaíonn sé foláireamh lánscáileáin; seinntear fuaim an bhonnáin ón leathanach aláraim",
  "SafeHome cảnh báo khẩn cấp": "Foláireamh éigeandála SafeHome",
  "Cảnh báo khẩn cấp ưu tiên cao trước khi mở toàn màn hình":
      "Foláireamh éigeandála ardtosaíochta sula n-osclaítear an lánscáileán",
  "SafeHome nhắc nhở toàn màn hình": "Meabhrúchán lánscáileáin SafeHome",
  "Nhắc nhở SafeHome toàn màn hình không âm thanh":
      "Meabhrúchán lánscáileáin SafeHome gan fuaim",
  "SafeHome nhắc nhở ưu tiên cao": "Meabhrúchán ardtosaíochta SafeHome",
  "Nhắc nhở SafeHome ưu tiên cao, không mở toàn màn hình":
      "Meabhrúchán ardtosaíochta SafeHome gan lánscáileán",
  "Tin nhắn mới trong các nhà SafeHome":
      "Teachtaireachtaí nua i mbailte SafeHome",
  "Hành động này sẽ thay đổi thời gian báo động của một số thiết bị hôm nay.\n\nBáo động của các thiết bị thuộc trường \"Nguy hiểm khẩn cấp\" và báo động ở chế độ \"Bảo vệ\" sẽ không bị ảnh hưởng bởi chức năng này.":
      "Athróidh an gníomh seo am aláraim roinnt gléasanna inniu.\n\nNí dhéanfaidh an ghné seo difear d’aláraim gléasanna sa chatagóir “Contúirt éigeandála” ná d’aláraim sa mhód “Cosaint”.",
  "🆘 \$count nhà nguy hiểm\$suffix": "🆘 \$count baile i mbaol\$suffix",
  "FCM token đã sẵn sàng, nhưng tính năng tự động khi rời nhà còn thiếu điều kiện.":
      "Tá an comhartha FCM réidh, ach tá riachtanas amháin in easnamh don Chosaint Uathoibríoch agus tú as baile.",
  "Đã kích hoạt SOS": "SOS gníomhachtaithe",
  "Phát hiện chập điện": "Braitheadh gearrchiorcad",
  "Phát hiện quá dòng": "Braitheadh forshruth",
  "Phát hiện quá áp": "Braitheadh forvoltas",
  "Thiết bị điện quá nhiệt": "Tá an gléas leictreach róthéite",
  "Tài khoản đang được sử dụng": "Tá an cuntas in úsáid",
  "Hiện tại tài khoản này đang đăng nhập trên một thiết bị khác. Nếu tiếp tục, tài khoản trên thiết bị đó sẽ tự đăng xuất.":
      "Tá an cuntas seo logáilte isteach ar ghléas eile faoi láthair. Má leanann tú ar aghaidh, logálfar an cuntas amach go huathoibríoch ar an ngléas sin.",
  "Tiếp tục đăng nhập": "Lean ar aghaidh ag logáil isteach",
  "Tài khoản đã được đăng nhập trên một thiết bị khác.":
      "Logáladh isteach sa chuntas ar ghléas eile.",
  "KHẨN CẤP": "ÉIGEANDÁIL",
  "CẦN CHÚ Ý": "AIRD DE DHÍTH",
  "THÔNG TIN": "EOLAS",
  "BÁO ĐỘNG": "ALÁRAM",
  "Cảm biến đã trở lại trạng thái an toàn.":
      "Tá an braiteoir ar ais i riocht sábháilte.",
  "Người dùng đã tắt cảnh báo.": "Mhúch úsáideoir an foláireamh.",
  "Khung giờ bảo vệ đã kết thúc.": "Tá an tréimhse chosanta thart.",
  "Cảnh báo tức thời đã tự kết thúc.":
      "Tháinig deireadh leis an bhfoláireamh láithreach go huathoibríoch.",
  "Điều kiện cảnh báo không còn hoạt động.":
      "Níl coinníoll an fholáirimh gníomhach a thuilleadh.",
  "Cảnh báo đã kết thúc: \$reason": "Tá an foláireamh thart: \$reason",
};
