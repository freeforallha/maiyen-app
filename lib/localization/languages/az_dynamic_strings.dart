const Map<String, String> azDynamicStrings = {
  "Bảo vệ - Tự động": "Mühafizə - Avtomatik",
  "Bảo vệ - Thủ công": "Mühafizə - Əl ilə",
  "Tự động": "Avtomatik",
  "Thủ công": "Əl ilə",
  "\$displayMemberName đã gia nhập nhà \"\$homeName\".":
      "\$displayMemberName \"\$homeName\" evinə qoşuldu.",
  "NGUY HIỂM": "TƏHLÜKƏ",
  "Nguy hiểm & Khẩn cấp": "Təhlükə və fövqəladə hal",
  "TẮT CÒI": "SİRENİ SÖNDÜR",
  "Tắt còi báo động?": "Həyəcan sireni söndürülsün?",
  "Còi vật lý sẽ dừng, nhưng cảnh báo vẫn tiếp tục cho đến khi sự cố được xử lý.\n\nBạn chắc chắn muốn tắt còi?":
      "Fiziki siren dayanacaq, lakin hadisə həll olunanadək xəbərdarlıq davam edəcək.\n\nSireni söndürmək istədiyinizə əminsiniz?",
  "Đã gửi lệnh tắt còi. Cảnh báo vẫn đang được theo dõi.":
      "Sireni söndürmək əmri göndərildi. Xəbərdarlıq hələ də izlənilir.",
  "Không tìm thấy cảnh báo đang hoạt động hoặc chưa thể gửi lệnh tắt còi.":
      "Aktiv xəbərdarlıq tapılmadı və ya sireni söndürmək əmri göndərilə bilmədi.",
  "TẮT CÒI TRONG NHÀ": "EVDƏKİ SİRENİ SÖNDÜR",
  "Đã tắt còi trong nhà. Cảnh báo vẫn tiếp tục cho đến khi được xử lý.":
      "Evdəki siren söndürüldü. Xəbərdarlıq həll olunanadək davam edəcək.",
  "\$name đang chuẩn bị gửi tin...": "\$name yazır...",
  "\$name1 và \$name2 đang chuẩn bị gửi tin...": "\$name1 və \$name2 yazır...",
  "\$name và \$otherCount người khác đang chuẩn bị gửi tin...":
      "\$name və daha \$otherCount nəfər yazır...",
  "Kênh báo động cũ để giữ tương thích":
      "Uyğunluğu qorumaq üçün köhnə həyəcan kanalı saxlanılıb",
  "SafeHome báo động toàn màn hình": "SafeHome tam ekran həyəcanı",
  "Mở cảnh báo toàn màn hình; âm còi phát từ trang báo động":
      "Tam ekran xəbərdarlığını açır; siren səsi həyəcan səhifəsindən səslənir",
  "SafeHome cảnh báo khẩn cấp": "SafeHome fövqəladə xəbərdarlığı",
  "Cảnh báo khẩn cấp ưu tiên cao trước khi mở toàn màn hình":
      "Tam ekran açılmazdan əvvəl yüksək prioritetli fövqəladə xəbərdarlıq",
  "SafeHome nhắc nhở toàn màn hình": "SafeHome tam ekran xatırlatması",
  "Nhắc nhở SafeHome toàn màn hình không âm thanh":
      "Səssiz SafeHome tam ekran xatırlatması",
  "SafeHome nhắc nhở ưu tiên cao": "Yüksək prioritetli SafeHome xatırlatması",
  "Nhắc nhở SafeHome ưu tiên cao, không mở toàn màn hình":
      "Tam ekran olmadan yüksək prioritetli SafeHome xatırlatması",
  "Tin nhắn mới trong các nhà SafeHome": "SafeHome evlərində yeni mesajlar",
  "Hành động này sẽ thay đổi thời gian báo động của một số thiết bị hôm nay.\n\nBáo động của các thiết bị thuộc trường \"Nguy hiểm khẩn cấp\" và báo động ở chế độ \"Bảo vệ\" sẽ không bị ảnh hưởng bởi chức năng này.":
      "Bu əməliyyat bu gün bəzi cihazların siqnal vaxtını dəyişəcək.\n\n“Fövqəladə təhlükə” kateqoriyasındakı cihazların siqnalları və “Mühafizə” rejimindəki siqnallar bu funksiyadan təsirlənməyəcək.",
  "🆘 \$count nhà nguy hiểm\$suffix": "🆘 \$count ev təhlükədədir\$suffix",
  "FCM token đã sẵn sàng, nhưng tính năng tự động khi rời nhà còn thiếu điều kiện.":
      "FCM tokeni hazırdır, lakin evdən çıxarkən avtomatik funksiya üçün hələ bir şərt çatışmır.",
  "Đã kích hoạt SOS": "SOS aktivləşdirildi",
  "Phát hiện chập điện": "Qısaqapanma aşkarlandı",
  "Phát hiện quá dòng": "Həddindən artıq cərəyan aşkarlandı",
  "Phát hiện quá áp": "Həddindən artıq gərginlik aşkarlandı",
  "Thiết bị điện quá nhiệt": "Elektrik cihazı həddindən artıq qızıb",
  "Tài khoản đang được sử dụng": "Hesab istifadə olunur",
  "Hiện tại tài khoản này đang đăng nhập trên một thiết bị khác. Nếu tiếp tục, tài khoản trên thiết bị đó sẽ tự đăng xuất.":
      "Bu hesab hazırda başqa cihazda daxil olub. Davam etsəniz, həmin cihazdakı hesabdan avtomatik çıxış ediləcək.",
  "Tiếp tục đăng nhập": "Daxil olmağa davam et",
  "Tài khoản đã được đăng nhập trên một thiết bị khác.":
      "Hesab başqa cihazda açılıb.",
  "KHẨN CẤP": "FÖVQƏLADƏ",
  "CẦN CHÚ Ý": "DİQQƏT TƏLƏB EDİR",
  "THÔNG TIN": "MƏLUMAT",
  "BÁO ĐỘNG": "HƏYƏCAN",
  "Cảm biến đã trở lại trạng thái an toàn.":
      "Sensor təhlükəsiz vəziyyətə qayıdıb.",
  "Người dùng đã tắt cảnh báo.": "İstifadəçi xəbərdarlığı söndürüb.",
  "Khung giờ bảo vệ đã kết thúc.": "Mühafizə müddəti başa çatıb.",
  "Cảnh báo tức thời đã tự kết thúc.": "Ani xəbərdarlıq avtomatik başa çatıb.",
  "Điều kiện cảnh báo không còn hoạt động.":
      "Xəbərdarlıq şərti artıq aktiv deyil.",
  "Cảnh báo đã kết thúc: \$reason": "Xəbərdarlıq başa çatdı: \$reason",
};
