...
void main() {
  ...
  final bottomBar = File(
    'lib/pages/home/home_bottom_bar.dart',
  ).readAsStringSync();
  ...
  test('banner có phiên bản Hub mới chỉ dành cho Owner', () {
    expect(
      coordinator,
      contains('ownerUid.isEmpty || ownerUid != uid.trim()'),
    );
  });
  ...
  test('Settings ngoài HomePage có dấu chấm than khi Owner cần update Hub', () {
    ...
    expect(
      bottomBar,
      contains('if (inviteCount > 0 || hubUpdateAttention)'),
    );
    expect(
      bottomBar,
      contains('inviteCount > 0 ? "\$inviteCount" : "!"'),
    );
    expect(homePage, contains('final hubUpdateAttention ='));
    expect(homePage, contains('isOwner() &&'));
    expect(
      homePage,
      contains('hubUpdateAttention: hubUpdateAttention'),
    );
  });
